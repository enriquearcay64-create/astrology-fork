...
def policy_manifest() -> Dict[str, object]:
    ...
    return {
        ...
        "house_policy": {
            "natal_psychological": "placidus",
            "whole_sign": "technique_specific_only",
            ...
            "whole_sign_natal_interpretation": False,
            "placidus_house_rulers": "traditional_cusp_sign_ruler_when_reliable",
        },
        ...
        "time_stability": {
            ...
            "stress_test_policy": "counterfactual stress tests are separate from declared uncertainty; Placidus and Whole Sign instability are recorded separately and applied only to their permitted uses",
            "placidus_house_ruler_cusp_sign_policy": "A Placidus house ruler is available only when the base cusp sign agrees with both declared-uncertainty endpoint cusps; stress tests are disclosure only.",
        },
        ...
        "secondary_points_core_semantics": {"natal_node_axis": True, "chiron": False, "lilith_mean": False},
        ...
        "counterweight_policy": "deterministic candidates share a body and use orb<=4deg, capped at one low-resistance and one friction candidate; Author retains only a candidate that materially qualifies the proposition or domain",
        "outer_planet_signature_policy": "Uranus, Neptune and Pluto require a safe link to Sun, Moon, Mercury, Venus, Mars or ASC/DSC/MC/IC to anchor a central personal signature; otherwise they remain contextual or modifying factors",
        "configuration_policy": "primary bodies only; exact configured natal aspects; absorb T-squares in grand crosses and grand trines in kites; stellium means 3+ primary bodies in one sign or reliable Placidus house; same-member sign/Placidus groups are one structural evidence family",
        ...
        "report_design_policy": "portable Markdown; executive/deep/technical separation; deep is Premium Complete; Placidus-led natal prose; Whole Sign only in named traditional techniques; signature-led human-first prose with mandatory adaptive coverage; timing translated before technical labels; deterministic technical appendix",
        "premium_workflow_policy": "one Codex request; Premium Author then Deterministic Provenance Guard then independent Premium Reviewer/Editor then Publication Guard; local CLI remains an offline/debug surface and no deterministic guard claims semantic proof",
        "premium_handoff_contract_version": PREMIUM_HANDOFF_CONTRACT_VERSION,
        "premium_birth_time_policy": "premium beta requires a known natal time; unknown-time charts remain eligible only for the limited safe deterministic reading",
