...
def humanization_instructions(language: str = "pt-BR") -> str:
    ...
    if language.startswith("pt"):
        return (
            ...
            "Ritmo editorial e recursos destacados: a maior parte do relatório deve ser prosa natural contínua. Use recursos editoriais (uma microcena dentro do parágrafo, um exemplo destacado, 2–4 bullets para itens paralelos, uma pergunta discriminativa ou uma síntese curta) apenas quando melhorarem materialmente a compreensão, retenção ou ritmo. microcenas hipotéticas curtas de 2–4 frases são bem-vindas em 3 a 5 dinâmicas centrais ou abstratas para tornar mecanismos autorizados reconhecíveis no cotidiano (como tomada de decisão, limites, conflito, trabalho ou dinheiro), nunca por quota, nunca rotuladas com tags em caixa alta e nunca como biografia inventada. Use bullets preferencialmente para manifestações paralelas, polos de tensão, comportamentos sob pressão ou escolhas práticas, e não para meramente repetir placements já citados no texto. "
            ...
            "No timing (Capítulo 18), comece pelo campo humano ativado e então apresente a técnica, aspecto/configuração, janela temporal, datas de exatidão/perfeição, fator natal ativado e interpretação humana sem previsões deterministas ou acontecimentos garantidos (evite 'portal decisivo', 'alinhamento de destino', 'abrirá portas', 'determinará as próximas décadas'). Quando várias interações natal-timing legais acrescentarem consequências distintas, desenvolva sua convergência e datas úteis; agrupe timing por campo humano e convergência, não por lista de técnicas. "
    ...
    return (
        ...
        "For the three or four leading dynamics, develop an inner mechanism, recognizable expression, distinction between integrated expression and defensive/under-pressure response, excess or false solution, material counterweight, and pathway of integration with another important factor when each genuinely adds meaning, never as a repeated formula. Prefer verbs, movement, decisions, and lived experience over repeated nominalizations such as function, available coordination, structure, direction, integration, criterion, or possibility. Depth may be visibly irregular when the chart earns it; similar section lengths call for editorial inspection, never a mechanical target. That focus organizes the opening; it does not cap the rest of the reading. In each domain, develop every distinct reader-relevant mechanism supported by authorised evidence, including a tension, counterweight, distinct consequence, or whole-chart integration when each adds meaning. Humanisation changes expression; it does not reduce what the chart understands. Do not expose routing vocabulary: write 'because Venus rules your fourth house', not 'house four routed to Venus'. "
        "Editorial rhythm and featured elements: the vast majority of the report must remain continuous natural prose. Use editorial elements (an embedded micro-scene, a separate example, 2–4 parallel bullets, a discriminative question, or a short synthesis) only when they materially improve comprehension, retention, or rhythm. Short hypothetical micro-scenes of 2–4 sentences are welcome in 3 to 5 central or abstract dynamics to make authorized mechanisms recognizable in everyday life (such as decision-making, boundaries, conflict, work, or money), never by quota, never labeled with uppercase tags, and never as invented biography. Use bullets preferably for parallel manifestations, tension poles, pressure behaviors, or practical choices, not merely repeating placements already in prose. "
        ...
        "For timing (Chapter 18), lead with the human field being activated, then show technique, aspect/configuration, window, exactness/perfection dates, activated natal factor, and human interpretation without deterministic prophecies or guaranteed events (avoid 'decisive portal', 'destiny alignment', 'will open doors', 'will determine decades'). When several legal natal-timing interactions add distinct human consequences, develop their convergence, natal linkage, and useful dates/windows without technical dumping; group timing by human field and convergence, not a technique list, and do not reduce the active chapter to one activation merely for brevity. "
...
def humanization_verifier_instructions(language: str = "pt-BR") -> str:
    ...
    if language.startswith("pt"):
        return (
            ...
            "rejeite e reescreva ativamente quando encontrar: (1) grandiosidade ou tom heroico inflado; (2) bajulação ou elogios desprovidos de lastro técnico; (3) certeza biográfica ou psicológica injustificada; (4) medicalização ou alegações fisiológicas específicas; (5) termos técnicos incorretos ou números/graus recalculados pelo LLM; (6) timing determinista com garantia de acontecimentos; (7) fórmulas genéricas tipo Barnum ou autoajuda rasa; (8) abstração vazia sem ancoragem concreta; (9) repetição vocabular e loops retóricos; (10) template smell e andaimes mecânicos previsíveis entre capítulos; (11) seções longas sem nenhum elemento concreto; (12) exemplos hipotéticos que não derivam da evidência do mapa. "
            "Tarefa editorial obrigatória: identifique de 3 a 5 mecanismos centrais ou abstratos cuja compreensão melhoraria materialmente com uma pequena situação concreta e certifique-se de que uma microcena hipotética de 2–4 frases esteja presente nesses pontos, sem criar quotas por capítulo. Revise bullets: certifique-se de que sejam usados para manifestações paralelas, polos de tensão, comportamentos sob pressão ou escolhas práticas, e não para meramente repetir placements. "
            "Nas dinâmicas centrais, exija desenvolvimento reconhecível do mecanismo sem impor uma fórmula de abertura ou parágrafo. Não aprove uma leitura só porque cada heading existe: verifique se mecanismos autorizados distintos foram desenvolvidos antes de o texto ser comprimido e revise toda omissão ou merge do reader_selection_plan: peça regeneração quando um caminho ou cluster acrescentar mecanismo humano distinto que o rascunho não desenvolve. Dê preferência a uma cadência humana: experiência, astrologia que ilumina, nuance e reflexão, preservando variação. "
