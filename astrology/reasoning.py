...
def humanization_instructions(language: str = "pt-BR") -> str:
    ...
    if language.startswith("pt"):
        return (
            ...
            "numa fórmula repetitiva. Esse foco organiza a abertura, não limita a profundidade do restante: em cada domínio, desenvolva todo mecanismo distinto e relevante que a evidência autorizada sustentar, incluindo tensão, contrapeso, consequência própria e integração com a arquitetura inteira quando eles realmente acrescentarem sentido. Humanizar muda a expressão, não reduz o que o mapa compreende. Prefira linguagem vivida a repetir registro semântico como função, coordenação disponível, estrutura, critério ou "
            "possibilidade. Mantenha precisão e cautela, mas diga os limites naturalmente, sem deixar a voz de QA aparecer no texto. Interpretação vem antes de coaching. "
            "No timing, comece pelo campo humano ativado e então apresente trânsito, profecção, progressão, arco ou ciclo; use somente candidatos tipados selecionados quando puderem sustentar um campo específico. Quando várias interações natal-timing legais acrescentarem consequências humanas diferentes, desenvolva a convergência, a ligação natal e janelas/datas úteis sem despejo técnico; não reduza o capítulo ativo a uma única ativação só por concisão. A integração final começa pela síntese de ordem superior do mapa, nunca por cronologia ou metadados de timeline, e ganha espaço para integrar mecanismos centrais, contradições, relação entre vida privada e expressão pública e timing quando forem distintos. Não use voz acadêmica, "
...
def humanization_verifier_instructions(language: str = "pt-BR") -> str:
    ...
    if language.startswith("pt"):
        return (
            ...
            "sem impor uma fórmula de abertura ou parágrafo. Não aprove uma leitura só porque cada heading existe: verifique se mecanismos autorizados distintos foram desenvolvidos antes de o texto ser comprimido. Dê preferência a uma cadência humana: experiência, astrologia que ilumina, nuance e reflexão, preservando variação. "
    ...
    return (
        ...
        "require recognizable development of the mechanism without imposing a fixed opening or paragraph formula. Do not approve merely because each heading exists: check that distinct authorised mechanisms were developed before prose was compressed. Prefer a human cadence of experience, illuminating astrology, nuance, and reflection, while preserving variation. Remove internal-QA wording, legalistic cautions, and repeated resource/risk scaffolding whenever natural language keeps the same boundary, but never replace astrological reasoning with generic self-help. Retain timing or developmental material only "
