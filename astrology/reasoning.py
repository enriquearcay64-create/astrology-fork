...
def humanization_instructions(language: str = "pt-BR") -> str:
    ...
    if language.startswith("pt"):
        return (
            "Escreva como alguém que entra no mapa ao lado da pessoa: caloroso, receptivo, elegante e psicologicamente claro, sem intimidade inventada. "
            ...
            "como 'você pode notar' e o ritmo repetitivo de recurso, risco e pergunta. Para as três ou quatro dinâmicas principais, desenvolva, quando realmente acrescentar sentido, mecanismo interno, "
            ...
            "numa fórmula repetitiva. Esse foco organiza a abertura, não limita a profundidade do restante: em cada domínio, desenvolva todo mecanismo distinto e relevante que a evidência autorizada sustentar, incluindo tensão, contrapeso, consequência própria e integração com a arquitetura inteira quando eles realmente acrescentarem sentido. A profundidade pode ser visivelmente irregular quando a carta a ganha; comprimentos parecidos pedem revisão editorial, não uma meta mecânica. Humanizar muda a expressão, não reduz o que o mapa compreende. Prefira verbos, movimentos, decisões e experiência vivida a repetir registro nominal como função, coordenação disponível, estrutura, direção, integração, critério ou "
            "possibilidade. Não leve vocabulário de roteamento ao leitor: diga, por exemplo, 'como Vênus rege sua quarta casa', não 'a casa quatro encaminhada a Vênus'. Microcenas hipotéticas curtas são bem-vindas quando tornam um mecanismo autorizado reconhecível, nunca por quota nem como biografia. Mantenha precisão e cautela, mas diga os limites naturalmente, sem deixar a voz de QA aparecer no texto. Interpretação vem antes de coaching. "
            "No timing, comece pelo campo humano ativado e então apresente trânsito, profecção, progressão, arco ou ciclo; use somente candidatos tipados selecionados quando puderem sustentar um campo específico. Quando várias interações natal-timing legais acrescentarem consequências humanas diferentes, desenvolva a convergência, a ligação natal e janelas/datas úteis sem despejo técnico; agrupe timing por campo humano e convergência, não por lista de técnicas, e não reduza o capítulo ativo a uma única ativação só por concisão. A integração final começa pela síntese de ordem superior do mapa, nunca por cronologia ou metadados de timeline, e ganha espaço para integrar mecanismos centrais, contradições, relação entre vida privada e expressão pública e timing quando forem distintos. Não use voz acadêmica, "
            ...
            "que exigem uma ReasonedSynthesis aprovada. Estruture o Premium com a introdução fixa do produto antes da abertura, depois abertura do mapa inteiro, exatamente os 16 headings canônicos do manifest e "
            "integração final. A introdução não é interpretação, não recebe proveniência nem conta como seção. Cada domínio disponível recebe tratamento próprio; low emphasis reduz prioridade, nunca vira um padrão de um parágrafo nem apaga mecanismos distintos. Não use quotas de palavras ou de parágrafos. Use somente as legal coverage "
            "paths e seu authorized_scope: um apoio não fabrica âncora e uma âncora estreita não autoriza todos os conceitos do heading. Em criatividade, Sol só sustenta vitalidade e Vênus prazer/valores: criatividade e expressão exigem rota legal da casa 5. Em corpo, Marte só sustenta ação/esforço; rotina, serviço e cuidado prático exigem rota legal da casa 6, e Marte não autoriza por si recuperação, regulação corporal ou saúde. Na direção de desenvolvimento, trate o eixo nodal completo — ambos os signos, ambas as casas e a tensão relacional — sem transformar Nodo Norte em Aquário na casa 1 em dependência de grupo. Explique um mecanismo "
    ...
    return (
        ...
        "counterweight, and interaction with another important factor when each genuinely adds meaning, never as a repeated formula. Prefer verbs, movement, decisions, and lived experience over repeated nominalizations such as function, available coordination, structure, direction, integration, criterion, or possibility. Depth may be visibly irregular when the chart earns it; similar section lengths call for editorial inspection, never a mechanical target. That focus organizes the opening; it does not cap the rest of the reading. In each domain, develop every distinct reader-relevant mechanism supported by authorised evidence, including a tension, counterweight, distinct consequence, or whole-chart integration when each adds meaning. Humanisation changes expression; it does not reduce what the chart understands. Do not expose routing vocabulary: write 'because Venus rules your fourth house', not 'house four routed to Venus'. Short hypothetical micro-scenes are welcome when they make an authorised mechanism recognizable, never by quota or as invented biography. Keep precision and caution, but express limits naturally rather than letting internal QA voice reach the reader. Interpretation comes before coaching. "
        "For timing, lead with the human field being activated and then name the transit, profection, progression, arc, or cycle; use selected typed candidates only when they support a specific field. When several legal natal-timing interactions add distinct human consequences, develop their convergence, natal linkage, and useful dates/windows without technical dumping; group timing by human field and convergence, not a technique list, and do not reduce the active chapter to one activation merely for brevity. Final integration begins with the map's higher-order synthesis, never timeline chronology or metadata, and has room to integrate central mechanisms, contradictions, private/internal structure with public expression, and current timing when these are distinct. Use no academic, legalistic, or internal-QA voice. Do not add a factor, biography, event, diagnosis or certainty. "
...
def humanization_verifier_instructions(language: str = "pt-BR") -> str:
    ...
    if language.startswith("pt"):
        return (
            ...
            "íntima sem presunção, de baixo jargão, com tradução imediata de termos astrológicos e interpretação antes de coaching; mantenha todo o relatório voltado ao leitor consistentemente no idioma solicitado, sem termos residuais de outro idioma; "
            ...
            "sem impor uma fórmula de abertura ou parágrafo. Não aprove uma leitura só porque cada heading existe: verifique se mecanismos autorizados distintos foram desenvolvidos antes de o texto ser comprimido e revise toda omissão ou merge do reader_selection_plan: peça regeneração quando um caminho ou cluster acrescentar mecanismo humano distinto que o rascunho não desenvolve. Dê preferência a uma cadência humana: experiência, astrologia que ilumina, nuance e reflexão, preservando variação. "
            ...
            "superficial, abertura que inventarie placements e conclusão que apenas resuma. Exija que a integração comece pelo que organiza a pessoa como um todo, não por timeline ou metadados. Compare cada tratamento com authorized_scope de sua legal path; um "
            ...
            "acrescente consequência própria do domínio, que a integração final desenvolva síntese memorável em vez de resumo, e preserve exatamente qualquer aviso determinístico indisponível. Antes de aprovar, julgue semanticamente se o mecanismo humano distinto de cada path represented está materialmente expresso nos blocos elegíveis; para merged, a convergência precisa ser material e não pode ser justificada apenas por planeta, regente, classe de raciocínio, operação ou tópico amplo compartilhado. Para omitted, ateste que nenhuma consequência humana distinta permanece. Python prova somente shape, ordem, IDs, hashes, syntheses aprovadas, provenance física de paragraphs/list items, legalidade individual de syntheses, ownership, timing e coverage determinística; não prova equivalência semântica. H3 deve ser revisado para orientação e significado, mas nunca conta para mandatory/domain/selection coverage. Revalide no relatório final o plano usando somente paragraphs/list items; se o Author paragraph/list for removido e restar apenas H3 sourced, rejeite. Exija hashes, ownership e source rows atualizados após qualquer edição de layout."
        )
    return (
        ...
        "voice with immediate plain-language translation of astrology and interpretation before coaching; keep the entire reader-facing report consistently in the requested language, without residual terms from another language; reject prose that is technically correct "
        ...
        "require recognizable development of the mechanism without imposing a fixed opening or paragraph formula. Do not approve merely because each heading exists: check that distinct authorised mechanisms were developed before prose was compressed, and inspect every omission or merge in the reader_selection_plan; request regeneration when a path or synthesis cluster adds a distinct human mechanism that the draft lacks. Prefer a human cadence of experience, illuminating astrology, nuance, and reflection, while preserving variation. Remove internal-QA wording, legalistic cautions, and repeated resource/risk scaffolding whenever natural language keeps the same boundary, but never replace astrological reasoning with generic self-help. Retain timing or developmental material only "
        ...
        "ending. Require final integration to begin with the whole person's higher-order organization, not timeline or metadata. Compare each treatment with the authorized_scope of its legal path; a supporting factor cannot create an anchor and a narrow scope cannot cover "
