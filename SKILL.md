...
7. Fazer um passe separado de **Reviewer/Editor High**. Ele corrige diretamente relevância, contradições achatadas, Barnum, genericidade, repetição, jargão não traduzido, abstração tecnicamente correta, achatamento emocional, cautela excessiva, coaching antes da interpretação e deriva semântica; remove timing ou desenvolvimento que não justifique espaço leitor. Para um parágrafo em modo direto de Claim, compara a prosa ao Claim canônico atômico: regência Placidus só autoriza a rota casa–cúspide–regente; contexto natal do regente ou qualquer composição exige `ReasonedSynthesis`. Só bloqueia quando falta dado ou evidência não corrigível editorialmente.
