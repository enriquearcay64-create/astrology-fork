...
def test_reader_selection_plan_order_agnostic_and_multi_path():
    """Verify R2: ReaderSelectionPlan is order-agnostic, Author-owned in production, and conservative in fallback."""
    import json
    ...
    # 1. In production, build_canonical_selection_plan fails closed without author_selection_plan
    with pytest.raises(ValueError, match="author_selection_plan is required"):
        build_canonical_selection_plan(manifest)

    # 2. Conservative fallback represents all legal paths without Python heuristics
    ...
    for d_id, paths in by_domain.items():
        assert all(p["decision"] == "represented" for p in paths), f"Fallback in {d_id} must represent all paths"

    # Order-agnostic verification under reversal
    shuffled_manifest = {
    ...
    shuffled_plan = build_canonical_selection_plan(shuffled_manifest, allow_conservative_fallback=True)
    ...
    # 3. Verified Author selection plan from benchmark validates with rich multi-path representations
    ...
    if bench_plan_path.exists():
        ...
        work_paths = auth_by_dom["work_vocation_visibility"]
        assert len(work_paths) == 2
        assert all(p["decision"] == "represented" for p in work_paths)
        # Money: Casa 2 and Casa 8 represented, topical Mars merged
        money_paths = auth_by_dom["money_resources_value"]
        ...
        # Rationales for merged/omitted paths must be substantive
        for d in author_plan["domains"]:
            for p in d["paths"]:
...
def test_author_bundle_provenance_guard_pass_with_v221():
    """Verify that author bundle with prospective plan and selection passes Provenance Guard."""
    import json
    from pathlib import Path
    handoff = prepare_premium_handoff(CHART_3_BIRTH)
    manifest = handoff["reader_domain_manifest"]
    ...
    res = validate_premium_author_bundle(CHART_3_BIRTH, bundle, prepared_handoff=handoff)
...
def test_semantic_block_reassignment_on_paragraph_swap():
    ...
    from tests.v221_fixtures import SAMPLE_CHART3_PROSE
    handoff = prepare_premium_handoff(CHART_3_BIRTH)
    manifest = handoff["reader_domain_manifest"]
    block_plan = plan_prospective_narrative_blocks(handoff, allow_conservative_fallback=True)
    ...
    sources_orig, sections_orig, _ = bind_prospective_plan_to_prose(SAMPLE_CHART3_PROSE, block_plan, manifest)
    ...
    swapped_report = SAMPLE_CHART3_PROSE.replace(p0 + "\n\n" + p1, p1 + "\n\n" + p0)
...
def test_reviewer_bundle_automatic_rebind_on_edited_report():
    ...
    author_bundle = build_author_bundle(
        handoff, SAMPLE_CHART3_PROSE, sources,
    ...
    edited_report = SAMPLE_CHART3_PROSE.replace(
...
def test_portuguese_roman_numerals_and_unaccented_house_matching():
    ...
    swapped_roman_report = SAMPLE_CHART3_PROSE.replace(p0 + "\n\n" + p1, roman_b0 + "\n\n" + roman_b1)
    ...
    swapped_unacc_report = SAMPLE_CHART3_PROSE.replace(p0 + "\n\n" + p1, unaccented_b0 + "\n\n" + unaccented_b1)
...
def test_semantic_attribution_avoids_incidental_cross_mention_pollution():
    ...
    polluted_report = SAMPLE_CHART3_PROSE.replace(
...
def test_selection_plan_order_invariance_across_diverse_fixtures():
    ...
    for key in ["A", "B", "C", "E", "F", "G", "H"]:
        ...
        manifest = analysis["reader_domain_manifest"]
        orig_plan = build_canonical_selection_plan(manifest, allow_conservative_fallback=True)
...
def test_portuguese_cardinal_and_prepositional_house_matching():
    ...
    card_report = SAMPLE_CHART3_PROSE.replace(p0 + "\n\n" + p1, cardinal_b0 + "\n\n" + cardinal_b1)
    ...
    prep_report = SAMPLE_CHART3_PROSE.replace(p0 + "\n\n" + p1, prep_b0 + "\n\n" + prep_b1)
...
def test_english_house_references_semantic_reassignment():
    ...
    en_report = SAMPLE_CHART3_PROSE.replace(p0 + "\n\n" + p1, en_b0 + "\n\n" + en_b1)
